...
inParameter( [] , _ ).
inParameter([H|T], ValList) :- member(H, ValList), inParameter(T, ValList).

all_different([]).
all_different([Head|Tail]):- \+member(Head, Tail), all_different(Tail).
...
inParameter(Solution, [1,2,3,4,5,6,7,8,9]),
